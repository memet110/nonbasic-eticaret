...
import { createDesign, createCollection, createProductType } from "./actions";
...
export function NewDesignForm({ 
  ...
  const [productType, setProductType] = useState(initialProductTypes?.[0]?.name || "T-Shirt");
  ...
  const handleAddProductType = async () => {
    ...
    try {
      const res = await createProductType(newProductTypeName);
      if (res.error) throw new Error(res.error);
      if (res.productType) {
        setProductTypes([...productTypes, res.productType]);
        setProductType(res.productType.name);
  ...
  const handleAddCollection = async () => {
    ...
    try {
      ...
      if (res.collection) {
        ...
        setSelectedCollection(res.collection.id);
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select 
                    ...
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:ring-black focus:border-black outline-none bg-white text-sm"
                  >
                    {productTypes.map(pt => (
